"""Run from the repository: .venv/bin/python examples/run_shanghai.py"""
import json
import os
import sys
...
chan = CChan(code='sh.000001', begin_time='2024-01-01',
             end_time=date.today().isoformat(), data_src=DATA_SRC.BAO_STOCK,
             lv_list=[KL_TYPE.K_DAY], config=config, autype=AUTYPE.NONE)
daily = chan[0]
...
plot.save2img(str(ROOT / 'output' / 'shanghai_daily.png'))
summary = {
...
(ROOT / 'output' / 'shanghai_summary.json').write_text(
